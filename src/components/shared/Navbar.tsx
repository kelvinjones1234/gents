...
import { Menu, Search, ShoppingBag, User, X, ArrowRight, ArrowLeft, Instagram, Facebook } from "lucide-react";
import { NAV_LINKS } from "@/app/constants/navigation"; // Assuming this is an array of objects like { name: "Dog", href: "/dog" }
  ...
  return (
    <>
      ...
      <div className="w-full bg-[#FAFAFA] text-foreground border-b border-gray-200 flex justify-between items-center py-2 px-4 md:px-12 z-30 relative">
        <button className="p-1 hover:opacity-60 transition-opacity" aria-label="Previous announcement">
          <ArrowLeft className="w-3.5 h-3.5 md:w-4 md:h-4 text-muted" strokeWidth={1.5} />
        </button>
        ...
        <button className="p-1 hover:opacity-60 transition-opacity" aria-label="Next announcement">
          <ArrowRight className="w-3.5 h-3.5 md:w-4 md:h-4 text-muted" strokeWidth={1.5} />
        </button>
      </div>
      ...
      <nav className="sticky top-0 z-40 w-full bg-background/90 backdrop-blur-md text-foreground border-b border-muted/20 mx-auto py-4 uppercase tracking-wide text-xs font-medium font-sans transition-colors duration-300">
        <div className="container-main flex justify-between items-center w-full px-6 md:px-12">
          ...
          <div className="hidden md:flex items-center justify-end space-x-6 w-1/3">
            ...
            <Link href={`/account/login`} className="hover:text-brand-sage transition-colors duration-200 flex items-center gap-1">
              <User className="h-4 w-4" strokeWidth={1.5} />
              <span className="sr-only lg:not-sr-only">Account</span>
            </Link>

            <button className="relative hover:text-brand-sage transition-colors duration-200 flex items-center gap-1 group">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-8 hide-scrollbar">
          {NAV_LINKS.map((link) => (
            ...
            >
              ...
              <ArrowRight className="w-4 h-4 text-muted/50 group-hover:text-foreground transition-colors duration-300" strokeWidth={1} />
            </Link>
          ))}

          {/* Custom Sale Link (To match your screenshot's red text) */}
          <Link
            href="/sale"
            onClick={() => setIsMobileMenuOpen(false)}
        ...
        <div className="px-6 pb-10 pt-8 shrink-0 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-5">
            <a href="#" className="flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase text-foreground hover:text-muted transition-colors">
            ...
            <a href="#" className="flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase text-foreground hover:text-muted transition-colors">
              {/* Custom tiny SVG for TikTok since Lucide doesn't have it natively in standard sets */}
          ...
          <button className="flex items-center gap-3 text-[10px] font-bold tracking-widest uppercase text-foreground hover:opacity-70 transition-opacity self-start">
            {/* Replace with an actual flag SVG or image if preferred */}
            <span className="text-sm leading-none border border-gray-200">🇳🇬</span> 
            NGN ₦
          </button>
        </div>
      </div>
